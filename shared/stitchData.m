...
%Initialize memory
D.time = zeros(n,1);
Names = {'state','control','contact','power','energy'};
...
%Special case - only used for optimization results:
if isfield(Data(1),'integrand')
    D.integrand = structInit(Data(1).integrand,n);
    for i=1:N
        IdxList = IdxBnd(i,1):IdxBnd(i,2);
        subNames = fieldnames(D.integrand);
        for k=1:length(subNames)
            D.integrand.(subNames{k})(IdxList) = ...
                Data(i).integrand.(subNames{k});
        end
    end
