...
addpath ../../GAIT_Walk; addpath ../../GAIT_Run;
...
INPUT.physical.coeff_friction = 0.99;
INPUT.physical.actuator_leg_saturate = 4; %2;   % (_)*(Mass*Gravity)
INPUT.physical.actuator_hip_saturate = 1.5; %0.8; % (_)*(Length*Mass*Gravity)
INPUT.physical.actuator_ank_saturate = 0.7; %0.2; % (_)*(Length*Mass*Gravity)

%%%% Constraints %%%%
INPUT.constraint.duration_single_stance = [0.05; 2];
INPUT.constraint.duration_double_stance = [0.05; 2];
INPUT.constraint.duration_flight = [0.05; 2];
INPUT.constraint.speed = 0.1*[1;1]; %[0.05; 2.0];
INPUT.constraint.step_distance = [0.2; 2.0];
INPUT.constraint.ground_slope = 0;
INPUT.constraint.ground_curvature = 0;
INPUT.constraint.center_clearance = 0;

%%%% Optimization %%%%
INPUT.optimize.solver = 'ipopt';   %{'ipopt', 'snopt'}
INPUT.optimize.tol_mesh = 1e2;
INPUT.optimize.tol_opt = 1e-4;
