...
%Get information necessary for computing contact forces at toe off
Control_End_D = input.phase(1).finalstate(IdxControlDouble);
P_Dyn = input.auxdata.dynamics;
P_Dyn.x2 = -STEP_DIST;
P_Dyn.y2 = 0;
[~, contactForces] = dynamics_double(END_D, Control_End_D, P_Dyn);
H2 = contactForces(:,3);
V2 = contactForces(:,4);
...
%%%% Contact forces == zero at toe-off
output.eventgroup(6).event = [V2, H2];  
