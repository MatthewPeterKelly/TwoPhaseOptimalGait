...
MAX_ITER = INPUT.optimize.max_iter;
...
auxdata.footClearance = INPUT.constraint.clearance;
...
auxdata.dynamics.u = INPUT.physical.coeff_friction;  %Between the foot and the ground
...
Max_Contact = 4*Leg_Max;    %A rough approximation for contact force bounds
...
auxdata.cost.w = INPUT.cost.weight;
...
P.Bnd(iphase).Path(:,1) = [0;Max_Contact]; % 0 < H1 + u*V1
P.Bnd(iphase).Path(:,2) = [0;Max_Contact]; % 0 < u*V1 - H1
P.Bnd(iphase).Path(:,3) = [0;Max_Contact]; % 0 < H2 + u*V2
P.Bnd(iphase).Path(:,4) = [0;Max_Contact]; % 0 < u*V2 - H2
%Columns 5:6 are for the absolute value constraints
...
P.Bnd(iphase).Path = zeros(2,7);
P.Bnd(iphase).Path(:,1) = [0;Max_Contact]; % 0 < H1 + u*V1
P.Bnd(iphase).Path(:,2) = [0;Max_Contact]; % 0 < u*V1 - H1
P.Bnd(iphase).Path(:,3) = [0; LEG_LENGTH(UPP)]; %Swing foot clears ground
%Columns 4:7 are for abs() constraints
...
mesh.method = 'hp-PattersonRao'; % {'hp','hp1'};
...
setup.nlp.options.maxiterations = MAX_ITER;  % (3000) IPOPT only
